...
2. **压缩**：阶段 2 只留 Fisher Overlap 和归一化后的 $D$、$C$，不做梯度冲突
3. **砍**：阶段 4 relearning、3B 全量
...
这两张图把参数侧和行为侧接起来了，正是最后那张对照表想要的东西，而且完全免费——数据在阶段 1–4 里已经产生了。**汇报时这是最有分量的两页。**
...
| 第 11 天 | 阶段 5 闭环验证，出两张散点图 |
| 第 12 天 | 阶段 4 relearning **或** 3B 上 2 个方法的一致性验证（二选一） |
| 第 13 天 | 补实验、统一出图 |
| 第 14 天 | 做 PPT |

头两天大概率都在填坑（装不上、跑不动、数字对不上），留出这个余量。

**关于 3B**：v1 的"坑"那节写了必须跑 3B，但排期表里没有位置。v2 明确把它降级为**第 12 天、2 个方法（建议 NPO + RMU）的跨模型一致性验证**，不是全套六个方法。如果第 12 天来不及，就在报告里写成 next step，不要两边都挂着。
