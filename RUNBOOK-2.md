...
**库自带一层更细的续跑，可以白用。** `src/evals/base.py` 的 `evaluate()` 在 `overwrite=False`（评测配置的默认值）时，先读 `output_dir` 下已有的 `TOFU_EVAL.json`，然后逐指标 `if metric_name in logs: skip already evaluated`。所以中断在评测中途，重跑只会补没算完的那几个指标，粒度比 `.done` 哨兵还细一级。

**但它只认 `output_dir`，不认模型。** 换了权重、指向同一个目录，它会直接把上一个变体的结果当成已算过返回，**不报错、不警告**。所以（`eval_output_isolation`）：

> 扫层、移植、relearning 的**每一个变体必须各用自己的 `paths.output_dir`**。

命名照 §1 的目录约定：`eval/<method>/restore_layer_<k>_cheap/`、`transplant_layer_<k>_cheap/`、`relearn_step_<n>_cheap/`。共用目录是这套流程里最容易产生「看起来跑完了、数字全是错的」的一个点。
...
│   │   ├── trainer/RMU_downproj.yaml   # 见 S5，避开命令行 regex 转义
...
├── results/
│   ├── snapshot.json
│   ├── gates/{gate_1a,gate_1b,gate_2,dry_runs}.json
...
│   └── figures/
└── notes/{pip_freeze.txt,gate_1b_deviation.md,eval_timing.md,schedule_recomputed.md}
```
...
| 10 | RMU_downproj 预检计数 ≠ 3，或 checkpoint-4 的 Δ≠0 张量数 ≠ 3 | `gate_2.RMU_downproj.verification` |
| 11 | 第 1 天三处 dry run 任一未通过 | `dry_runs.items` |
...
**三份都 dry run**，验证 `eval=<name>` 能压过 `experiment` 里的 `override /eval: tofu`，且各自输出的指标集正确。每份记录一次实际耗时到 `notes/eval_timing.md`，后面排期按它算。这是 `dry_runs.items` 的第一项。
...
## S3b — 第 1 天 dry run 集中验证（硬停）

**前置**：S3 完成。

三处需要 dry run 的地方原本分散在 S3、S5、S16，最晚的一处要到第 11 天才验——那时候发现搞不定就没时间改了。全部前置到第 1 天，和 Gate 1a 同一天做完（`dry_runs.when`）。

| id | 验什么 | 归属步骤 | 做法 |
|---|---|---|---|
| `eval_group_override` | `eval=tofu_cheap` 能否压过 `experiment` 里的 `override /eval: tofu`，且指标集正确 | S3 | 已在 S3 完成，勾掉即可 |
| `rmu_downproj_config` | `trainer=RMU_downproj` 能否正确加载，预检计数 == 3 | S5 | 起训后跑到 optimizer 构造完即中断，**不必训完** |
| `relearn_data_config` | 自建 data 配置（`path=json` + `data_files`）能否被 `finetune` trainer 正常加载 | S16 | 造 5 条假数据，`max_steps=2` 跑通即可 |

`relearn_data_config` 是三处里最脆的：它要照库的 `TOFU_QA_*` 数据配置改写 `hf_args`，格式对不上时报错信息通常指向 datasets 内部，不容易定位。

**任一未通过 → STOP-ASK #11。** 不得带着未验证的配置进入 S5。

**三处全通过后**（`dry_runs.after_all_pass`）：用 `notes/eval_timing.md` 里 `tofu_full` / `tofu_oppoint` / `tofu_cheap` 三份的实测单次耗时，把整张排期重算一遍，写入 `notes/schedule_recomputed.md`。

粗估的评测量（`training.verified_from_trainer_state` 回填后按实际步数重算）：

| 配置 | 次数 | 来源 |
|---|---|---|
| `tofu_oppoint` | ≈ 84 | S6，7 个 run × 12 |
| `tofu_full` | ≈ 35 | S6 第 10 步留档 + S13 |
| `tofu_cheap` | ≈ 173 | S11 + S12 + S14 + S16 |

`tofu_oppoint` 单次不便宜——`model_utility` 要在 retain 400 + real_authors 100 + world_facts 100 共 600 条上生成。若实测耗时使总量明显超出排期，在这里就要决定砍什么（按 v3.1 的优先级那节），不要等到第 8 天。

**产物**：`../results/gates/dry_runs.json` + `notes/schedule_recomputed.md`
...
**RMU_downproj 走配置文件，不走命令行 override**（`gate_2.RMU_downproj.trainer_config`）。regex 里有 `(`、`|`、`\.` 三种元字符，命令行要同时过 shell 和 Hydra 两层转义，风险没必要担。新建 `scripts/configs/trainer/RMU_downproj.yaml`：

```yaml
defaults:
  - RMU
  - _self_          # 必须在后，否则 RMU 的 method_args 会盖掉下面这段
...
单引号包住，YAML 不做反斜杠转义，写法与 `configs/trainer/RMU.yaml` 里注释那行完全一致。命令行只加 `trainer=RMU_downproj`。

**注意不要给 `|` 加反斜杠**——`\|` 在 Python `re` 里是字面竖线，模式匹配不到任何参数，训练等于没动，最后会被误判成"到不了操作点 A"。

**可训练矩阵数的两级校验**（`gate_2.RMU_downproj.verification`），两级都**不看 `requires_grad`**：

- **预检**（训练前）：用同一个 regex 在 `model.named_parameters()` 上跑 `re.fullmatch` 计数，必须 == `expected_trainable_matrices`
- **实锤**（`checkpoint-4` 存出后）：state_dict 逐张量与 target 比较，Δ≠0 的张量必须恰好 `expected_trainable_matrices` 个

任一不符 → **STOP-ASK #10**。

> 为什么不能查 `requires_grad`：`RMU.create_optimizer()` 的最后一行是 `self._freeze_all_params(self.model, True)`，optimizer 构造完之后所有参数的 `requires_grad` 又被设回 `True`。训练开始时打印一定是全部参数，这个检查永远不可能通过。要看 optimizer 真实持有的量，只能查 `trainer.optimizer.param_groups`，但那里没有参数名——所以用上面两级替代。
...
- `trainer=finetune`，训练数据用 `--config-dir` 里自建的 `scripts/configs/data/` 配置指向 `finetune_file`（照库的 `TOFU_QA_*` 数据配置改 `hf_args` 为 `path=json` + `data_files`）。**该配置已在 S3b 的 `relearn_data_config` 验过**，这里直接用
- 每个评测点各用自己的 `paths.output_dir`（`eval/<method>/relearn_step_<n>_cheap/`），见 §0.1 的目录隔离规则
